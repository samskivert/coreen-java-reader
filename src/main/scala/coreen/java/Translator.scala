...
import com.sun.source.util.{TreePath, TreePathScanner}
...
class Translator (_types :Types) extends TreePathScanner[Unit,ArrayBuffer[Elem]] {
  ...
  private def trimDoc (text :String) =
    text.split(LineSeparator).map(_.trim).map(snipStar).filter(
      _.length != 0).mkString(LineSeparator)
...
object Translator {
  ...
  val IgnoredAnnotations = Set("Override", "SuppressWarnings")

  def kindForSym (sym :Symbol) = sym match {
  ...
  def targetForTypeSym (sym :Symbol) :String = sym match {
